...
def plot_convergence(inform, train_idxs=None, ax=None):
    ...
    assert len(inform.shape) == 2, "Inform must be two dimensional, N x M where N is the number of runs, and M the number of samples."
    ...
    ax.plot(range(len(inform[:,train_idxs])), 
...
def plot_multiple_runs(sizes, data, color="green", label=None, ax=None, alpha=1.0, linestyle=None):
    """
    ...
    ax.plot(sizes, np.mean(data, axis=1), linestyle=linestyle, color=color, label=label, alpha=alpha)
...
def plot_comparison(sizes, informativeness_data, random_data, bad_data=None, ax=None):
    informativeness_data, random_data = ensure_array(informativeness_data), ensure_array(random_data)
    
    ax.plot(sizes, np.mean(informativeness_data, axis=1), color="green", label="Informative")
    ax.fill_between(sizes, 
                np.mean(informativeness_data, axis=1) - np.sqrt(np.var(informativeness_data, axis=1)), 
                np.mean(informativeness_data, axis=1) + np.sqrt(np.var(informativeness_data, axis=1)),
                color="green",
                alpha=0.2)

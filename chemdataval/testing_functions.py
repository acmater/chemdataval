...
def active_test(
    ...
    regressor = ActiveLearner(
        ...
        X_training=X[seed],
        y_training=Y[seed].reshape(-1, 1),
    )

    # You want the scores to contain a baseline score value
    idxs, scores = list(seed), [regressor.score(*test)]
    for _ in range(queries):
        query_idx, query_instance, sub_idx = regressor.query(
            X, pool_idxs, train_index, density_vector, β=β
        )
...
def fold_testing(
    ...
    with warnings.catch_warnings():
        ...
        for kf_idx, item in enumerate(kf.split(X, categorical)):
            ...
            for strategy in strategies.keys():
                results[strategy] = {
                    ...
                    for key, arr in zip(
                        ["idxs", "scores"],
                        [
                            np.zeros((random_runs, queries + seed_size), dtype=np.int),
                            np.zeros((random_runs, queries + 1)),
                        ],
                    )
                }
            ...
            density_arrays = {
                "Information Density": 1 / (1 + cdist(X̂, X̂, metric="euclidean")),
            ...
            density_vectors = {
                name: np.mean(density_array[train_index, :][:, train_index], axis=1)
                for name, density_array in density_arrays.items()
            }
            ...
            for idx in tqdm.tqdm(range(random_runs)):
                ...
                for strategy, strategy_func in strategies.items():
                    ...
                    # Sanity check tests
                    assert (
                        len(np.intersect1d(idxs, test_index)) == 0
                    ), f"Fold {kf_idx+1} - {strategy} There is overlap between the training and testing indices."
                    assert len(idxs) == len(
                        np.unique(idxs)
                    ), f"Fold {kf_idx+1} - {strategy} There are duplicates in the training indices selected."
                    assert (
                        len(np.intersect1d(idxs[:seed_size], idxs[seed_size:])) == 0
                    ), f"Fold {kf_idx+1} - {strategy} The model is re-selecting points in the seed dataset."
